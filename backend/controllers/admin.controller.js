...
module.exports.updateWelcomeSection = async (req, res) => {
  try {
    ...
    // Find the welcome section by ID
    const getWelcomeSection = await Welcome.findById(id);
    if (!getWelcomeSection) {
      return res.status(404).json({ message: 'Welcome Section not found' });
    }

    // If there's a file, handle the image update
    if (req.file) {
    ...
    const welcome = await Welcome.findByIdAndUpdate(id, { $set: updatedData }, { new: true });

    if (welcome) {
      res.status(200).json({ welcome });
    } else {
      res.status(400).json({ message: 'Failed to update Welcome Section' });
    }
  } catch (err) {
    console.error('Update Welcome Section error:', err);
    res.status(500).json({ message: 'Internal Server Error' });
  }
};
