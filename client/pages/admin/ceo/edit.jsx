...
function EditCeoMsg(){
   ...
   async function handleSubmit(e){
        ...
        if(image){
            formData.append('ceo',image);
        }
        // console.log(formData);
        const response = await dispatch(patchCeoMsg({id, data : formData}))
        if(patchCeoMsg.fulfilled.match(response)){
            toast.success('Ceo Message Updated Sucessfully!', {
                position: "top-right",
                autoClose: 1000,
    ...
    if(nextPage){
        return <Navigate to={'/admin/ceoMessage'} ></Navigate>
    }
    return (
        <>
