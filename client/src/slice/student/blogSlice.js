...
export const fetchBlogs = createAsyncThunk('fetchBlogs', async () => {
  const response = await axiosApi.get('student/blog',{
    headers: {
...
export const postBlog = createAsyncThunk('postBlog', async ({data}) => {
    try {
      ...
      const response = await axiosApi.post(`student/blog`, data, {
        headers: {
          'Content-Type': 'multipart/form-data',
          'Authorization': `Bearer ${getCookie('token')}`
        }
...
export const patchBlog = createAsyncThunk('patchBlog', async ({id, data}) => {
  try {
    console.log(data);
    console.log(id);
    const response = await axiosApi.patch(`student/blog/${id}`, data, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Authorization': `Bearer ${getCookie('token')}`
      }
...
export const deleteBlog = createAsyncThunk('deleteBlog', async ({id}) => {
  try {
    const response = await axiosApi.delete(`student/blog/${id}`, {
      headers: {
        'Content-Type': 'multipart/form-data',
        'Authorization': `Bearer ${getCookie('token')}`
      }
